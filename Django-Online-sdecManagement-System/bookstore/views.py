...
from .models import User, Book,DeleteRequest, Feedback
...
def uabook(request):
    ...
    if request.method == 'POST':
        ...
        a = Book(title=title, year=year, publisher=publisher,
                 date=today, time=time, uploaded_by=username, user_id=user_id)
        a.save()
...
def controll(request):
    ...
    if request.method == 'POST':
        ...
        latest_book.controle = controle
        latest_book.time2 = time2
        latest_book.save()  # Save the updated entry
        return redirect('lmbook')
    else:
...
def aabook(request):
    ...
    if request.method == 'POST':
        ...
        a = Book(title=title, author=author, year=year, publisher=publisher,
                 desc=desc, date=today, time=time, uploaded_by=username, user_id=user_id)
        a.save()
        messages.success(request, 'Book was uploaded successfully')
